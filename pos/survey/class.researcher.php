...
{	
	...
	public function register($firstname,$lastname,$email,$upass,$code)
	{
		...
		{		
			...
			$password = md5($upass);
			$stmt = $this->conn->prepare("INSERT INTO tbl_researcher(FirstName,LastName,userEmail,userPass,tokenCode, Date_Created) 
		...
		try
		{
			...
			if($stmt->rowCount() == 1)
			{
				if($userRow['userStatus']=="Y")
				{
					...
					{
						$_SESSION['researcherSession'] = $userRow['userID'];
						return true;
					}
					else
					{
	...
	{
		session_destroy();
		$_SESSION['researcherSession'] = false;
	}
