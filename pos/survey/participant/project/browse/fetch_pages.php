...
$sql=mysqli_query($connecDB,"SELECT * FROM tbl_researcher_project ORDER BY id DESC LIMIT $position, $item_per_page");
...
<div class="therow">
  ...
  echo '<img src="'.BASE_PATH.'/projects/uploads/'.$row['project_image'].'" class="img-circle"/>';

}else{

 echo '<img src="'.BASE_PATH.'/projects/uploads/thumbnail.jpg" class="img-circle"/>';
}
  echo '</div>
  <div class="col-lg-7"><p><h4>'.$row['Name'].'</h4></p><p>'.$row['Headline'].'</p>
  <p>Payout: $'.$row['Pay'].' for '.$row['Minutes'].' minutes </p></div>
   <div class="col-lg-3"><a href="../../../projects/'.$row['Category'].'/?id='.$row['ProjectID'].'"><button type="button" class="btn-request">View Project</button></a> </div>
</div>
</div>
