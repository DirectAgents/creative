...
if(isset($_POST['btn-submit']))
{
	...
	$stmt = $user->runQuery("SELECT userID FROM tbl_researcher WHERE userEmail=:email LIMIT 1");
	...
	if($stmt->rowCount() == 1)
	{
		...
		$stmt = $user->runQuery("UPDATE tbl_researcher SET tokenCode=:token WHERE userEmail=:email");
