...
$sqlresearcher=mysqli_query($connecDB,"SELECT * FROM tbl_researcher WHERE userID = '".$_SESSION['researcherSession']."'");
//$resultresearcher=mysql_query($sqlresearcher);
$rowresearcher = mysqli_fetch_array($sqlresearcher);


$stmt = $researcher_home->runQuery("SELECT * FROM wepay WHERE researcher_id=:uid");
...
$sql="SELECT * FROM wepay WHERE researcher_id = '".$_SESSION['researcherSession']."' AND refunded = '' ORDER BY order_by DESC ";
...
$sql="SELECT DISTINCT(checkout_find_date) FROM wepay WHERE researcher_id = '".$_SESSION['researcherSession']."' ORDER BY order_by DESC ";
$result=mysql_query($sql);
//$row=mysql_fetch_array($result);
...
<?php if($rowresearcher['credit_card_id'] == '') { ?>
