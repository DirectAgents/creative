...
$results = mysql_query("SELECT userID,FirstName, LastName,Meetupchoice, Age, Gender, Height, City, State, Status, Ethnicity, Smoke, Drink, Diet, Religion, Education, Job, Bio 
  FROM tbl_participant WHERE userID IN (SELECT userID FROM tbl_project_request WHERE ProjectID = '".$_GET['id']."'
  AND Accepted_to_Participate = 'Accepted') AND Meetupchoice RLIKE '[[:<:]]".$Meetupchoice."[[:>:]]' $theage $thegender $theheight $thecity
...
<div class="therow">
  <div class="col-lg-2">';
echo '<a href="profile/?id='.$row2['userID'].'&p='.$_GET['id'].'">';
...
$results = mysql_query("SELECT userID,FirstName, LastName,Meetupchoice, Age, Gender, Height, City, State, Status, Ethnicity, Smoke, Drink, Diet, Religion, Education, Job, Bio 
  FROM tbl_participant WHERE userID IN (SELECT userID FROM tbl_project_request WHERE ProjectID = '".$_GET['id']."'
  AND Accepted_to_Participate = 'Pending') AND Meetupchoice RLIKE '[[:<:]]".$Meetupchoice."[[:>:]]' $theage $thegender $theheight $thecity 
  $thestatus $theethnicity $thesmoke $thedrink $thediet $thereligion $theeducation $thejob 
  ORDER BY userID DESC LIMIT $position, $item_per_page");
...
{
echo "<div class='no-participants'>";
echo "You don't have any pending projects to be accepted by participants";
echo "</div>";
