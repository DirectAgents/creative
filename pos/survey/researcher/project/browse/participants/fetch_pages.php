...
$sql = mysql_query("SELECT * FROM tbl_researcher_project WHERE ProjectID = '".$_GET['id']."' AND researcherID='".$_SESSION['researcherSession']."'");
...
$results = mysql_query("SELECT userID,FirstName, LastName,Meetupchoice, Age, Gender, Height, City, State, Status, Ethnicity, Smoke, Drink, Diet, Religion, Education, Job, Bio FROM tbl_participant 
WHERE Meetupchoice RLIKE '[[:<:]]".$Meetupchoice."[[:>:]]' $theage $thegender $theheight $thelocation $thestatus $theethnicity $thesmoke $thedrink $thediet $thereligion $theeducation $thejob ORDER BY userID DESC LIMIT $position, $item_per_page");
...
$sql = "SELECT * FROM tbl_participant_request WHERE ProjectID != '".$_GET['id']."' ";
//$sql = "SELECT * FROM tbl_participant_request WHERE userID !='".$row2['userID']."' AND ProjectID != '".$_GET['id']."' ";
//$sql = "SELECT * FROM tbl_participant_request WHERE ProjectID = '".$_GET['id']."' ";
