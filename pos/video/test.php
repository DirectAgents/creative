...
$sql = mysqli_query($connecDB,"SELECT * FROM tbl_users WHERE userID ='206'");
...
if($row['ProfileImage'] == 'Google'){ 
    $image =  '<img src="'.$row['google_picture_link'].'" class="thumb-lg img-circle" alt="img">';
} 

if($row['ProfileImage'] == 'Facebook'){

$image = '<img src="https://graph.facebook.com/'.$row['facebook_id'].'/picture?type=large" style="border-radius:50%; height:120px; width:120px" alt="img">';
}

if($row['ProfileImage'] == 'Linkedin'){
   $image =  '<img src="'.$row['linkedin_picture_link'].'" class="thumb-lg img-circle" alt="img">';
} 



$sql_requested = "SELECT * FROM tbl_users WHERE userID ='206'";  
...
$from = new SendGrid\Email("Request to connect", "support@valifyit.com");
