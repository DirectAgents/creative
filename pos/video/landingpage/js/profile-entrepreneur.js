...
$( "#contact-us" ).on( "submit", function(e) {  
 ...
 e.preventDefault();
    ...
    var email = $("input[name='fm_email']").val();
    var message = $("textarea[name='fm_message']").val();

    if (name == '') {
        $('input[name=fm_name]').css('border','1px solid red'); 
        
        proceed = false;
    }else{
      $('input[name=fm_name]').css('border','1px solid green'); 
      proceed = true;
    }

     if (email == '') {
        $('input[name=fm_email]').css('border','1px solid red'); 
        
        proceed = false;
    }else{
      $('input[name=fm_email]').css('border','1px solid green'); 
      proceed = true;
    }

     if (message == '') {
        $('textarea[name=fm_message]').css('border','1px solid red'); 
        
        proceed = false;
    }else{
      $('textarea[name=fm_message]').css('border','1px solid green'); 
       proceed = true;
    }
    ...
    $.ajax({
            url: url_link_original+"contact_form.php", 
            method: "POST",
            data: $(this).serialize(),
            dataType: "html",
            success: function(response) {
