...
if(isset($_GET['id']) && isset($_GET['code']))
{
	...
	$stmt = $participant->runQuery("SELECT * FROM tbl_customer WHERE userID=:uid AND tokenCode=:token");
	...
	if($stmt->rowCount() == 1)
	{
		if(isset($_POST['btn-reset-pass']))
		{
			...
			{
				$password = md5($cpass);
				$stmt = $participant->runQuery("UPDATE tbl_customer SET userPass=:upass WHERE userID=:uid");
