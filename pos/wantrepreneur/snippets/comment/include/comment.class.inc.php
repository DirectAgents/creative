...
class c5t_comment
{
    ...
    function get_rating($id)
    {
        ...
        $sql = "SELECT  comment_rating
                FROM    " . C5T_COMMENT_TABLE . "
                WHERE   comment_id = ?";
        if ($db = c5t_database::query($sql, array($id))) {
            $res = $db->fetchRow();
            if (PEAR::isError($res)) {
    ...
    function put()
    {
        ...
        if ($res = c5t_database::insert('comment', $data)) {
            if ($c5t['enable_moderation'] != 'Y' and $c5t['_post']['rating'] > 0) {
                c5t_identifier::add_rating($identifier_id, $c5t['_post']['rating']);
            }
    ...
    function status($id, $status)
    {
        ...
        $where = "comment_id = ?";
        $where_data = array($id);
        return c5t_database::update('comment', $data, $where, $where_data);
    }
    ...
    function identifier_id()
    {
        ...
        if ($id = c5t_database::next_id('identifier')) {
            ...
            $data = array(  'identifier_id'     => $_GET['id'],
    ...
    function select_identifier($identifier)
    {
        ...
        $sql = "SELECT  *,
                        ...
                        identifier_rating_number AS rating_number
     ...
     * Display comment data
     */
    function latest_comments($number_of_comments)
    {
        global $c5t;

        $result = c5t_comment::cache_list();

        if (count($result) <= $number_of_comments) {
            return $result;
        }

        return array_slice($result, 0, $number_of_comments);
    }

    /**
     * Rating images
     */
    function get_rating_images($data)
    {
        global $c5t;
        $images = array();
        $rating_value = 0;
        if ($data['identifier_rating_number'] > 0) {
            $rating_value = round($data['identifier_rating_value'] / $data['identifier_rating_number'], 1);
        }
        for ($i = 1; $i <= (int) $c5t['rating_top_value']; $i++)
        {
            $type = 'empty';
            if (($i-1+0.5) <= $rating_value) {
                $type = 'half';
            }
            if ($i <= $rating_value) {
                $type = 'full';
