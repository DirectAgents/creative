...
$results = mysqli_query($connecDB,"SELECT * FROM tbl_participant WHERE userID NOT IN (SELECT userID FROM tbl_participant_feedback_participated WHERE ProjectID = '".$_GET['id']."') AND userID NOT IN (SELECT userID FROM tbl_feedback_request WHERE ProjectID = '".$_GET['id']."') AND userID NOT IN (SELECT userID FROM tbl_feedback_upcoming WHERE ProjectID = '".$_GET['id']."') AND userID NOT IN (SELECT userID FROM tbl_feedback_recent WHERE ProjectID = '".$_GET['id']."') AND userID NOT IN (SELECT userID FROM tbl_feedback_archived_participant WHERE ProjectID = '".$_GET['id']."' AND Met = 'yes') AND userID NOT IN (SELECT userID FROM tbl_feedback_archived_startup WHERE ProjectID = '".$_GET['id']."' AND Met = 'yes') $theage $thegender $theheight $thecity $thestatus $theethnicity $thesmoke $thedrink $thediet $thereligion $theeducation $thejob $interests $languages ORDER BY userID DESC LIMIT $position, $item_per_page");
...
echo "<h3>";
echo "No new potential Participants available!";
echo "</h3>";
echo "<h4>";
